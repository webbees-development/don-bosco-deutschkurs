    ...
    methods: {
      ...
      getSteps() {
        ...
        if (this.isOverflown(this.carousel)) {
          ...
          let gap = parseInt(getComputedStyle(slides[1]).getPropertyValue("--gap")); // rem
          const globalFontSize = parseInt(getComputedStyle(document.querySelector("html")).getPropertyValue("font-size"));
          gap = gap * globalFontSize;
  
          for (let slide in slides) {
            if (local_steps[slide] + slides[slide].clientWidth - 2 + gap > this.carousel.scrollWidth - this.carousel.clientWidth) break;
      ...
      initButtonVisibility(position) {
        if (this.isOverflown(this.carousel)) {
          ...
          if ((position ? position : this.getPosX(this.carousel)) === this.steps[0]) {
            this.btnPrev.classList.remove("show");
            this.btnPrev.classList.add("hide");
          } else if ((position ? position : this.getPosX(this.carousel)) === this.steps[this.steps.length - 1]){
      ...
      handleStartDrag(event) {
        if (event.cancelable) {
          ...
          this.startMousePos = event.pageX ? event.pageX : event.changedTouches[0].pageX;
      ...
      handleMoveDrag(event) {
        ...
        const movedValue = this.rangeMovedValue(Math.round(this.startCarouselPos + this.startMousePos - (event.pageX ? event.pageX : event.changedTouches[0].pageX)));
        this.carousel.setAttribute("style", `transform: translateX(-${movedValue}px) !important;`);
      },
      handleStopDrag(event) {
        ...
        const movedValue = this.rangeMovedValue(Math.round(this.startCarouselPos + this.startMousePos - (event.pageX ? event.pageX : event.changedTouches[0].pageX)));
        this.carousel.setAttribute("style", `transform: translateX(-${movedValue}px);`);
      ...
      getClosestSlidePos(currentPos) {
        ...
        for (let step in this.steps) {
          if (this.steps[step] >= currentPos) {
            if (this.steps[step] - currentPos < currentPos - this.steps[step - 1]) {
              return this.steps[step];
            } else {
              return this.steps[step - 1];
            }
